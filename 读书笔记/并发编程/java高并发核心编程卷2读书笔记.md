...
一切的前提是当前锁是偏向锁的时候，并且只有两条线程争抢，如果存在线程的竞争，那么就会发现cas替换线程id是失败的，那么就会发生偏向锁的撤销以及锁膨胀的操作，在膨胀成轻量级锁之后第二条线程参与竞争，如果此时的对象被锁定那么就等到这个对象释放锁，之后就在线程栈帧中创建锁记录，并且用cas尝试替换对象mark word中的ptr_to_lock_record（锁记录指针）更新为抢锁线程栈帧中 Lock Record 的地址，如果成功了就说明这个线程拥有了和这个对象锁，然后该标记00，失败就肯定不止两个线程竞争，继续膨胀

轻量级锁的执行过程：latch.await();

在线程进入临界区代码块之前，如果**这个对象锁没有被线程占用的话**，首先在枪锁线程的栈帧中**建立一个锁记录**，这个里面存储的是对象mark word中的**拷贝**：
...
然后就进行自旋CAS的操作，将对象头中的Mark word中的锁记录指针**更新为栈帧中Lock Record**的地址，如果执行**成功**了的话，那么这个线程就拥有了这个对象锁，那么锁标志位就变为了00，这个对象就属于轻量级锁状态

在这之后还要处理两个善后的事情：

因为mark word被CAS更新之后，包含锁对象信息的旧值会被返回，需要抢锁线程找一个地方**将旧的 Mark Word 值暂存起来**

1. 这些在Mark world中的旧值存放到哪里呢？存放到当前线程的栈帧记录的 Displaced Mark Word这个里面，这是起到了备份的作用，目的是当线程结束后用来**恢复**之前的Mark word中记录，在说一下就是当锁升级后，对象的MW中就不存储基础的信息了比如说hashcode等信息，如果锁释放的时候，锁降级之后这些信息就从当前线程的栈帧中还原了，因为轻量级锁中已经不存储基础信息了

2. 将锁记录中的owner指针指向锁记录

   ![](https://github.com/JOYBOY-777/ReadStudyNote/blob/main/javaimg/java%E9%AB%98%E5%B9%B6%E5%8F%91%E6%A0%B8%E5%BF%83%E7%BC%96%E7%A8%8B%E5%8D%B7%E4%BA%8C%E5%9B%BE%E7%89%87/2-12.png?raw=true)
...
```java
        ...
        {   
            ...
            //循环完毕
            latch.countDown();
            //目的是让线程释放锁但是还要执行run方法，要知道执行完syn代码块之后线程就释放锁了，
            for (int j = 0; ; j++) {
                //每一次循环等待1ms
                sleepMilliSeconds(1);
            }
...
目的是让线程释放锁但是还要执行run方法，要知道执行完syn代码块之后线程就释放锁了，但是还让他一直执行不结束来让后面的latch.await();来结束他（很巧妙）来创造出对象没有被线程占用的环境，**虽然这个线程释放了锁，但是线程还在执行中没有结束还在执行run方法**，这种情况就是既存在线程的竞争又存在内置锁没有被锁定的情况，后面就是用CAS替换操作了，00是轻量级锁
...
其实我感觉这个轻量级锁，是当枪锁线程遇到的对象锁没有被锁定的情况下进行的，并且是进行CAS自旋锁，并不是比较耗费性能的重量级锁，当线程结束同步代码块后，轻量级锁进行释放操作，从而变成了**无锁状态 01**

轻量级锁的分类：

* 普通自旋锁：当有线程发现对象锁被占用，无法用CAS自旋的方式去占有锁的时候，就在原地傻傻的等待，一直进行自旋，直到上一个线程释放对象锁，这是才进行CAS的抢占
* 自适应自旋锁：就是会根据JVM的动态调整，比如你这个线程之前获得过这个对象锁，那么可能以后还会偏向这个枪锁的线程，并不是无限的自旋，而是得到JVM偏爱，从而更有可能的获取锁

轻量级锁的膨胀：

​	总的来说使用CAS自旋的话，虽然会消耗CPU的资源，但是速度很快，但是一旦**空自旋的线程变多**那么堆CPU的消耗就更大，所以就进行锁的膨胀，升级为**重量级锁**

**重量级锁核心原理**：

首先对象锁膨胀到重量级锁的时候，这时每个对象中就会存在一个叫做Monitor的东西。这个东西就像是一个监视器，包含了 Object 实例和 Class实例，并且这个监视器同对象一起进行创建和销毁，它的主要特点是：

1. 是一个运行许可，一个线程在进入临界区代码块执行资源的时候，就会获得这个许可，然后在离开的时候返还
2. 这个监视器拥有唤醒的机制，可以让正在持有锁的线程去放弃锁，从而进入阻塞的状态，那么其他占有锁的线程就可以去发送信号来唤醒
...
![](https://github.com/JOYBOY-777/ReadStudyNote/blob/main/javaimg/java%E9%AB%98%E5%B9%B6%E5%8F%91%E6%A0%B8%E5%BF%83%E7%BC%96%E7%A8%8B%E5%8D%B7%E4%BA%8C%E5%9B%BE%E7%89%87/synchronized%E9%94%81%E5%8D%87%E7%BA%A7%E8%BF%87%E7%A8%8B.png?raw=true)

![](https://github.com/JOYBOY-777/ReadStudyNote/blob/main/javaimg/java%E9%AB%98%E5%B9%B6%E5%8F%91%E6%A0%B8%E5%BF%83%E7%BC%96%E7%A8%8B%E5%8D%B7%E4%BA%8C%E5%9B%BE%E7%89%87/%E4%B8%89%E7%A7%8D%E9%94%81%E5%AF%B9%E6%AF%942-6.png?raw=true)



**线程间通讯**

总的来说就是线程之间进行相互的配合从而完成一个任务

之前的例子中的问题：无论是生产者还是消费者，这两个活动的异步任务在调用共享数据中的方法时都会进行条件的判空，说白了就是做无用的数据区询问，那么我们就可以wait-notify的方式进行控制：
...
**详解 wait 方法、notify 方法原理**

wait方法：让当前的线程进行阻塞并等待进行唤醒，并且使用的话一定要放在同步块中
