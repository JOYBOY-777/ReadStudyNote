...
目的是让线程释放锁但是还要执行run方法，要知道执行完syn代码块之后线程就释放锁了，但是还让他一直执行不结束来让后面的latch.await();来结束他（很巧妙）来创造出对象没有被线程占用的环境，**虽然这个线程释放了锁，但是线程还在执行中没有结束还在执行run方法**，这种情况就是既存在线程的竞争又存在内置锁没有被锁定的情况，后面就是用CAS替换操作了
