...
* 无锁状态：java对象刚刚创建的时候，还没有线程来争抢的状态，这时候的mark word的形态是0 01
...
* 偏向锁状态：指的是一段同步代码快一直被一个线程所访问，那么这个线程就会**自动获取**对象锁，如果内置锁是偏向状态的话，当有另外一个线程来竞争时，优先使用**这个锁偏向的线程**，并且这个线程在执行该锁关联的同步代码时**不需要做任何的检查和切换**，这种情况下在**竞争不激烈**的时候效率很高
